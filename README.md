...
### With ```go install```
You can download and install EpsilonFetch with the ```go install``` command. Simply run: 
```bash
go install -ldflags="-s -w" github.com/Moritisimor/EpsilonFetch/cmd/epsilonfetch@latest
```

The compiled binary can then be found in the ```bin``` folder of your ```GOROOT```. Usually this is ```~/go/bin```.

### Manual Cloning and Compilation
